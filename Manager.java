...
public class Manager {
  ...
  public static BufferedImage getImage(String file) {
    try {
      return Transparency.colorTrans(ImageIO.read(Class.forName("Manager").getResourceAsStream(file)), 0xFF000000, 0x00000000);
    } catch (Exception e) {
      e.printStackTrace();
    }
    return null;
            ...
            for (int i = 0; i < 4; ++i) {
              if (!Ghost.ghost[i].dead) {
                if (Ghost.ghost[i].insAway) {
                  if (Ghost.ghost[i].isBlinking() && frame == 1) boardG.drawImage(ghost[5][1], Ghost.ghost[i].x + 5, Ghost.ghost[i].y + 45, null);
                  else boardG.drawImage(ghost[4][0], Ghost.ghost[i].x + 5, Ghost.ghost[i].y + 45, null);
                } else boardG.drawImage(ghost[i][frame], Ghost.ghost[i].x + 5, Ghost.ghost[i].y + 45, null);
              }
