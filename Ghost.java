...
public class Ghost {
  ...
  static boolean away;
  static int timer2;//Syncs everything (used by Manager)
  static int count;//Determines when to release ghost (incremented by Board.removePill())
  static int released;
  ...
  public void instanceUpdate() {
    if (!dead) {
      if (insAway) {
        if (isBlinking() && Manager.frame == 1) Manager.boardG.drawImage(Manager.ghost[5][1], x + 5, y + 45, null);
  ...
  public int getFastestRoute() {
    ...
    if (dir != 3 && Board.square[blockXp2][blockY] != 1 &&
      Board.square[blockXp2][blockY + 1] != 1 && checked[blockXp1][blockY] != true) new End(blockXp1, blockY, 1, 1);
    if (dir != 4 && Board.square[blockX][blockY + 2] != 1 &&
      Board.square[blockXp1][blockY + 2] != 1 && checked[blockX][blockY + 1] != true) new End(blockX, blockY + 1, 2, 2);
    if (dir != 1 && Board.square[blockXm1][blockY] != 1 &&
      Board.square[blockXm1][blockY + 1] != 1 && checked[blockXm1][blockY] != true) new End(blockXm1, blockY, 3, 3);
    if (dir != 2 && Board.square[blockX][blockY - 1] != 1 &&
      Board.square[blockXp1][blockY - 1] != 1 && checked[blockX][blockY - 1] != true) new End(blockX, blockY - 1, 4, 4);
    while (true) {
